...
class AttentionConv(nn.Module):
    ...
    def forward(self, x, abs_x, deg, idx):
        ...
        # q : B, C, N
        # k, v : B, C, N
        # qTk : (B, N, C) x (B, C, N) = B, N, N
        # qTkV : (B, N, N) x (B, N, C) = B, N, C 

        q_out = q_out.squeeze(-1)

        out_all = torch.matmul(torch.transpose(q_out, 1, 2), k_out_all) # B, N, N 
        out_all = F.softmax(out_all, dim=-1) # B, N, N

        out_all = torch.matmul(out_all, torch.transpose(v_out_all, 1, 2)) # B, N, C
        out_all = out_all.permute(0, 2, 1).unsqueeze(-1) # B, C, N -> B, C, N, 1
