...
from .attention_NL_S import AttentionConv
...
class DGCNN_Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x, abs_x, deg1, idx1 = get_neighbors(x, k=self.k, deg=self.deg, delta=self.delta, neighbors=self.neighbors, bn=self.deg_bn1, layer=0) # b, 64, 1024, 20
        #print("1")
        x1 = self.conv1(x, abs_x, deg1, idx1) # b, 64, 1024
        x1 = self.act1(self.bn1(x1)).squeeze(3)

        #print("2")
        x, abs_x, deg2, idx2 = get_neighbors(x1, k=self.k, deg=self.deg, delta=self.delta, neighbors=self.neighbors, bn=self.deg_bn2, layer=1) # b, 64, 1024, 20
        x2 = self.conv2(x, abs_x, deg2, idx2) # b, 64, 1024
        x2 = self.act2(self.bn2(x2)).squeeze(3)

        #print("3")
        x, abs_x, deg3, idx3 = get_neighbors(x2, k=self.k, deg=self.deg, delta=self.delta, neighbors=self.neighbors, bn=self.deg_bn3, layer=2) # b, 64, 1024, 20
        x3 = self.conv3(x, abs_x, deg3, idx3) # b, 128, 1024
        x3 = self.act3(self.bn3(x3)).squeeze(3)

        #print("4")
        x, abs_x, deg4, idx4 = get_neighbors(x3, k=self.k, deg=self.deg, delta=self.delta, neighbors=self.neighbors, bn=self.deg_bn4, layer=3) # b, 64, 1024, 20
