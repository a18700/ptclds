...
class AttentionConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, groups=1, bias=False, rpe=False, deg=False, scale=False, return_kv=False, layer=0):
        ...
        if self.deg:
            ...
            self.query_conv = nn.Conv2d(in_channels//2, self.out_channels, kernel_size=1, bias=bias)
            self.key_conv = nn.Conv2d(in_channels, self.out_channels, kernel_size=1, bias=bias)
            self.value_conv = nn.Conv2d(in_channels, self.out_channels, kernel_size=1, bias=bias)
    ...
    def forward(self, x, abs_x, deg, idx):
        ...
        q_out = q_out.view(batch, self.groups, self.out_channels // self.groups, npoints, -1) # B, G, C//G, N, 1
        k_out = q_out.view(batch, self.groups, self.out_channels // self.groups, npoints, -1) # B, G, C//G, N, K
        v_out = q_out.view(batch, self.groups, self.out_channels // self.groups, npoints, -1) # B, G, C//G, N, K
       
        k_out_all = k_out[:,:,:,:,0] # B, G, C//G, N
        v_out_all = v_out[:,:,:,:,0] # B, G, C//G, N
        ...
        if self.scale:
            ...
            out = q_out * k_out # B, G, C//G, N, K

        out = F.softmax(out, dim=-1) # B, G, C//G, N, K

        out = torch.einsum('bgcnk,bgcnk -> bgcn', out, v_out) # b, G, C//G, N, K
        ...
        out_all = torch.matmul(torch.transpose(q_out, 2,3), k_out_all) # B, G, N, N 
        out_all = F.softmax(out_all, dim=-1) # B, G, N, N

        out_all = torch.matmul(out_all, torch.transpose(v_out_all, 2, 3)) # B, G, N, C//G
